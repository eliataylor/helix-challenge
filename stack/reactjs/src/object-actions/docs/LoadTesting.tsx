...
const APIPerformanceDashboard = () => {
  ...
  const getTimeChip = (time: number, label: string) => {
    ...
    if (time < 3003) return <WarningChip label={label} size="small" />;
  ...
  return (
    <Box mt={1}>
      ...
      <Typography variant="subtitle1">
        Base URL: {data.base_url ?? "https://localapi.helix.ai"}
      </Typography>
      <Typography variant="subtitle1">
        ...
        {activeTab === 2 && (
          <Box>
            <Typography gutterBottom={true}>1. Limit returned fields to only those requested by client with query param: <a target={"_blank"}
                                                                                                                            href={"https://github.com/eliataylor/objects-actions/blob/main/stack/django/helixai_app/serializers.py#L138"}>github.com/.../django/helixai_app/serializers.py#L138</a></Typography>
            <Typography gutterBottom={true}>2. Ease serializer on foreign keys that can be loaded separately: <a target={"_blank"}
                                                                                                                 href={"https://github.com/eliataylor/objects-actions/blob/main/stack/django/helixai_app/serializers.py#L25"}>github.com/.../django/helixai_app/serializers.py#L25</a></Typography>
            <Typography gutterBottom={true}>3. Replace queryset and `search_filter` with SQL / Stored Procedure: <a target={"_blank"}
                                                                                                                    href={"https://github.com/eliataylor/objects-actions/blob/main/stack/django/helixai_app/views.py#L113"}>github.com/.../django/helixai_app/views.py#L113</a></Typography>
          </Box>
        )}
        {activeTab === 0 && (
          <Box>
                      ...
                      {sortedEndpoints.map((endpoint) => {
                        ...
                        return operations[0].time > 0 ? (
                          <TableRow key={endpoint.api}>
                            <TableCell><a href={`https://localapi.helix.ai${endpoint.api}`} target={"_blank"}>{endpoint.api}</a></TableCell>
                ...
                <Grid container spacing={4}>
                  ...
                  <Grid item xs={12} md={6} lg={4}>
                    <Box>
                      ...
                      <Box sx={{ display: "flex", alignItems: "center" }}>
                        <Typography sx={{ mr: 2 }}>p95 &lt; 3003ms:</Typography>
                        {p95ResponseTime < 3003 ? (
                          <SuccessChip label="Passed" size="small" />
                        ) : (
                          <ErrorChip label="Failed" size="small" />
              ...
              <CardContent>
                ...
                <Grid container spacing={4}>
                  <Grid item xs={12} md={4}>
                    ...
                    <Typography>{data.base_url ?? "https://localapi.helix.ai"}</Typography>
                  </Grid>
                  <Grid item xs={12} md={4}>
