...
export const OAFormDrugComparisons: React.FC<OAFormProps<"DrugComparisons">> = ({ onSuccess }) => {

  const { renderField, handleSubmit, handleDelete, errors, navItem, entity, syncing } = useForm<"DrugComparisons">();
  ...
  async function saveEntity() {
    handleSubmit().then((newentity) => {
      ...
      enqueueSnackbar(`DrugComparisons saved`);
    }).catch(error => {
      console.error(error);
  ...
  return (
    <AlternatingList container spacing={4} p={1} justifyContent={'space-between'} wrap={"wrap"} >
      			<Grid item xs={12} >
				{renderField(TypeFieldSchema["DrugComparisons"]["url_alias"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["DrugComparisons"]["drug"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["DrugComparisons"]["comparison_criteria"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["DrugComparisons"]["expires"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["DrugComparisons"]["ai_answer"], 0, {fullWidth:true})}
			</Grid>
...
export default OAFormDrugComparisons;
