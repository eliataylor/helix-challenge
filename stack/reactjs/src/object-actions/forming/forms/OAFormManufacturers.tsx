...
export const OAFormManufacturers: React.FC<OAFormProps<"Manufacturers">> = ({ onSuccess }) => {

  const { renderField, handleSubmit, handleDelete, errors, navItem, entity, syncing } = useForm<"Manufacturers">();
  ...
  async function saveEntity() {
    handleSubmit().then((newentity) => {
      ...
      enqueueSnackbar(`Manufacturers saved`);
    }).catch(error => {
      console.error(error);
  ...
  return (
    <AlternatingList container spacing={4} p={1} justifyContent={'space-between'} wrap={"wrap"} >
      			<Grid item xs={12} >
				{renderField(TypeFieldSchema["Manufacturers"]["name"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["Manufacturers"]["location"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["Manufacturers"]["contact_info"], 0, {fullWidth:true})}
			</Grid>
...
export default OAFormManufacturers;
