...
export const OAFormSideEffects: React.FC<OAFormProps<"SideEffects">> = ({ onSuccess }) => {

  const { renderField, handleSubmit, handleDelete, errors, navItem, entity, syncing } = useForm<"SideEffects">();
  ...
  async function saveEntity() {
    handleSubmit().then((newentity) => {
      ...
      enqueueSnackbar(`SideEffects saved`);
    }).catch(error => {
      console.error(error);
  ...
  return (
    <AlternatingList container spacing={4} p={1} justifyContent={'space-between'} wrap={"wrap"} >
      			<Grid item xs={12} >
				{renderField(TypeFieldSchema["SideEffects"]["name"], 0, {fullWidth:true})}
			</Grid>
...
export default OAFormSideEffects;
